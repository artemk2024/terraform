...
resource "aws_db_subnet_group" "subnet_db" {
  name       = "subnet-db"
  subnet_ids = [aws_subnet.k8s_subnet_public1_eu_west_1a.id, aws_subnet.k8s_subnet_public2_eu_west_1b.id]
...
resource "aws_security_group" "k8s-db-sg" {
  name        = "k8s-db-sg"
  description = "k8s-db-sg"
  vpc_id      = aws_vpc.k8s.id
  ...
  ingress    {
      ...
      to_port          = 5432
      protocol         = "tcp"
      cidr_blocks      = ["212.90.172.68/32", "54.80.194.73/32"]
  }
